...
### 第四阶段：动态头像功能（forth.md）

**目标**：根据用户档案中的性别自动切换聊天界面用户头像。

**改动要点**：

1. **全局常量 `PICTURE_DIR`**
   - 基于 `__file__` 的绝对路径，保证跨环境兼容
   - 头像文件：`boy.jpg`（男）、`girl.jpg`（女）、`doctor.jpg`（医生）

2. **`update_chat_avatar(username)` 函数**
   - 读取 `DietManager.profile.gender` 字段
   - 男 → `boy.jpg`，女 → `girl.jpg`
   - 医生头像固定 `doctor.jpg`
   - **降级链**：目标缺失 → 尝试另一性别 → 都缺失则留空（无崩溃）

3. **四处事件链追加 `.then()`**
   - 登录按钮：登录 → 初始化聊天 → **更新头像**
   - 注册按钮：注册 → 初始化聊天 → **更新头像**
   - 退出登录：退出 → 清空聊天 → **重置头像**（默认男）
   - 档案更新：保存档案 → **实时刷新头像**（修改性别后无需重新登录）
